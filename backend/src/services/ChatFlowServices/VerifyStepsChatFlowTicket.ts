...
import canSendAutoAttendanceMessage from "../../helpers/canSendAutoAttendanceMessage";
...
const sendWelcomeMessage = async (
  ...
  if (
    flowConfig?.configurations?.welcomeMessage?.message &&
    (await canSendAutoAttendanceMessage(ticket))
  ) {
    const messageData = {
  ...
  if (
    ...
    if (ticket.chatFlowId) {
      ...
      if (stepCondition && !ticket.isCreated) {
        ...
        const canSendInitialBot = ticket.isCreated
          ? await canSendAutoAttendanceMessage(ticket)
          : true;
        if (canSendInitialBot) {
